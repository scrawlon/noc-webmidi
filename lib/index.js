  ...
  ' 1:0': [
    {
      ...
      'range': [0, rangeValues$l.distortion.driveType.length - 1],
      'rangeValues': rangeValues$l.distortion.driveType,
      'defaultValue': 0
    }
  ],
  ' 1:1': [
...
function updateMidiStatus(midi, config) {
  ...
  const { midiConnectionWrapper } = config;
  const midiConnectionStatusBox = midiConnectionWrapper && document.querySelector(midiConnectionWrapper);
...
function getMidiComponents(requestedMidiMessageType) {
  ...
  switch (midiMessageType) {
    case 'cc':
      ...
      midiMessageTypeValues = cc.midiCCs;
      break;
    case 'nrpn':
      components = nrpn.nrpnComponents;
      midiMessageTypeValues = nrpn.midiNRPNs;
      break;
    default:
...
export { getMidiComponents, initWebMidi, midiChannels, sendWebMidiEvent };
